 ...
 * <p>
 * Content providers for grid tree viewers must implement the {@link ITreeContentProvider}
 * interface.
 * <p>
 * <b>The current implementation does not support lazy content providers.</b>
 * </p>
 */
public class GridTreeViewer extends AbstractTreeViewer {
  ...
  /**
   * Creates a grid tree viewer on a newly-created grid control under the given parent. The grid
  ...
  /**
   * Creates a grid tree viewer on the given grid control. The viewer has no input, no content
   * provider, a default label provider, no sorter, and no filters.
   *
   ...
   * @param style the style bits to use for the new row
   * @param rowIndex the index at which the new row should be created under the parent
   * @return ViewerRow the new row
   */
   ...
   *
   * @param parentOrTreePath the parent element, the input element, or a tree path to the parent
   *          element
    ...
    preservingSelection( new Runnable() {
      ...
      public void run() {
        ...
        if( removedPath != null ) {
          ...
          if( removed ) {
            setSelection( new TreeSelection( ( TreePath[] )oldSelection
              .toArray( new TreePath[ oldSelection.size() ] ), getComparer() ), false );
          }
        }
      }
    } );
